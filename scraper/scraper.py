...
def add_new_listings(rss, c, geocode):
	...
	link = item.link.nextSibling
        ...
        c.execute('''SELECT COUNT(*) FROM listings WHERE guid = ?''', 
[guid])
        ...
        if c.fetchone()[0] <= 0:
            details = get_listing_details(link, geocode)
            c.execute('''INSERT INTO listings (guid, link, pubdate, 
address, price, lat, lng) VALUES(?, ?, ?, ?, ?, ?, ?)''', 
[guid, link, pubdate, 
details['address'], details['price'], details['lat'], details['lng']])
